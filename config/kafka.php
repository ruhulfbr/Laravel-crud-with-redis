...
return [
    ...
    /*
     | Kafka consumers belonging to the same Consumers group share a group id.
     | The consumers in a group then divides the topic partitions as fairly amongst themselves as possible by
     | establishing that each partition is only consumed by a single Consumers from the group.
     | This config defines the Consumers group id you want to use for your project.
     */
    ...
    /*
     | After the Consumers receives its assignment from the coordinator,
    ...
    /*
     | If you set enable.auto.commit (which is the default), then the Consumers will automatically commit offsets periodically at the
